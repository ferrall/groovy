...
- **Global scope**: `/Users/adar/Code/groovy/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/adar/Code/groovy/.claude/notes/`)
...
2. Glob `/Users/adar/Code/groovy/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/adar/Code/groovy/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/adar/Code/groovy/.claude/notes/` — works in any directory
